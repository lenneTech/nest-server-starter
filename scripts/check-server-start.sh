...
LOG_FILE=$(mktemp -t nest-server-starter-check.XXXXXX)
...
cleanup() {
  if [[ -n "${SERVER_PID:-}" ]]; then
    ...
    # on the child — it was the original hang. Nest can take a moment to
...
NSC__PORT=$FREE_PORT NODE_ENV=local node dist/src/main.js >"$LOG_FILE" 2>&1 &
SERVER_PID=$!
