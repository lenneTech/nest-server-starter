...
function toFixCommand(kind, cmd) {
  ...
  if (kind === "lint") {
    ...
    // Plain --fix only. --fix-suggestions may change behaviour: the no-console
    // suggestion deletes every console.log in the linted files.
...
export function killTreePlan(pid, signal, platform = process.platform) {
  return platform === "win32"
    ? { args: ["/PID", String(pid), "/T", "/F"], command: "taskkill" }
    : { signal };
}
