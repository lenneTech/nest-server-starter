 ...
 * developer's data: `tests/global-setup.ts` drops whatever NSC__MONGOOSE__URI
 ...
 *
 * The defect, kept as prose rather than as an `@regression` tag: this project has no
 * `tests/regression-mutations.json` and no mutation runner, so a `@seen-failing` line would name
 * a registry that does not exist. Upstream (@lenne.tech/nest-server) carries the same test with
 * the tag and the registered mutation `safe-test-db-pattern-unanchored`.
 *
 * To see this suite fail on purpose, restore the unanchored form in
 * tests/db-lifecycle.reporter.ts:
 *
 *     export const SAFE_TEST_DB_PATTERN = /(e2e|ci|test|acctest)/i;
 *
 * Every name in the refuse list below marked "witness" then flips to accepted.
 */
describe('SAFE_TEST_DB_PATTERN', () => {
  ...
  const ACCEPTED: [label: string, name: string][] = [
    ['e2e base (config.env.ts)', 'nest-server-starter-e2e'],
    ['ci base (config.env.ts)', 'nest-server-starter-ci'],
    ['per-run (global-setup.ts)', 'nest-server-starter-e2e-run-1783062000000-p12345'],
    ...
    ['per-fork (tests/setup.ts)', 'nest-server-starter-e2e-run-1783062000000-p12345-w3'],
    ['derived (deriveTestDbUri)', 'nest-server-starter-e2e-run-1783062000000-p12345-w3-tg-ba-resolver'],
    ...
    ['derived + truncated', 'nest-server-starter-e2e-run-1787490466261-p82-c2fb4f5a'],
    ['legacy leftover', 'nest-server-starter-e2e-setup-1783062745355'],
  ...
  it('is stateless — no /g flag, so repeated .test() cannot desync via lastIndex', () => {
    ...
    expect(SAFE_TEST_DB_PATTERN.test('nest-server-starter-e2e')).toBe(true);
    expect(SAFE_TEST_DB_PATTERN.test('nest-server-starter-e2e')).toBe(true);
  });
  ...
  it('still recognises the per-run databases global-setup.ts generates', () => {
    ...
    const generated = `nest-server-starter-e2e-run-${Date.now()}-p${process.pid}`;
 ...
 * condition, and `lt dev` points NSC__MONGOOSE__URI at exactly such a database.
 */
describe('NON_DISPOSABLE_DB_PATTERN / isDroppableTestDb', () => {
  ...
  it('does not refuse any name this project generates', () => {
    ...
    const generated = [
      ...
      'nest-server-starter-e2e-run-1783062000000-p12345',
      'nest-server-starter-e2e-run-1783062000000-p12345-w3',
      'nest-server-starter-e2e-run-1783062000000-p12345-w3-tg-ba-resolver',
      'nest-server-starter-e2e-run-1787490466261-p82352-w7-tg-ba-reso-c2fb4f5a',
      'nest-server-starter-e2e-setup-1783062745355',
      'nest-server-starter-e2e',
      'nest-server-starter-ci',
    ];
...
describe('isStaleTestDb', () => {
  const base = 'nest-server-starter-e2e';
  ...
  it('RUN_DB_PATTERN is end-anchored while isStaleTestDb is not — the asymmetry is load-bearing', () => {
    // onTestRunEnd returns early unless NSC__MONGOOSE__URI names a RUN database, so a per-fork name
...
describe('deriveTestDbUri', () => {
  const original = process.env.NSC__MONGOOSE__URI;

  afterEach(() => {
    if (original === undefined) {
      delete process.env.NSC__MONGOOSE__URI;
    } else {
      process.env.NSC__MONGOOSE__URI = original;
    }
  });

  it('derives from this run database and keeps the query string', () => {
    process.env.NSC__MONGOOSE__URI = 'mongodb://127.0.0.1/nest-server-starter-e2e-run-1783062000000-p12345?w=1';
    expect(deriveTestDbUri('mt')).toBe('mongodb://127.0.0.1/nest-server-starter-e2e-run-1783062000000-p12345-mt?w=1');
  });
  ...
  it('produces a name the cleanup will actually collect', () => {
    ...
    process.env.NSC__MONGOOSE__URI = 'mongodb://127.0.0.1/nest-server-starter-e2e-run-1783062000000-p12345';
  ...
  it('truncates past MongoDB’s 63-character limit without losing the marker', () => {
    process.env.NSC__MONGOOSE__URI = 'mongodb://127.0.0.1/nest-server-starter-e2e-run-1787490466261-p82352-w7';
