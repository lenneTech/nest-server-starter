...
export default class DbLifecycleReporter {
  async onTestRunEnd(
    ...
    if (reason !== 'passed' || unhandledErrors.length > 0) {
      ...
      // tests/setup.ts), NOT in `${dbName}` itself — the main process names the base, but every fork
      // suffixes it with its pool id. List the fork DBs so a developer debugging a failure connects
      // to the right ones. Best-effort: fall back to the base name if the listing fails.
    ...
    try {
      ...
      try {
        ...
        // from aborted runs of older code. The 1h age guard protects a
        // concurrently running old-code suite.
        ...
        for (const { name } of databases) {
          ...
          if (name.startsWith(`${dbName}-`)) {
            // DB derived from THIS run's name — the run is over.
            stale = true;
          } else if (name === base) {
